...
def yield_redis_msg_until(breaker):
...
def yield_socketmsg_until(namespace = '/',breaker = lambda: False):
    for m in yield_redis_msg_until(breaker):
        if m['type'] == 'message':
...
def yield_room_msg_until(room = None,breaker = lambda: False):
